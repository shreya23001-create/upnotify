...
export default async function TrackerDomainPage({
  ...
  const [checks, incidents, uptime30, uptime7] = await Promise.all([
    ...
    <div className="tracker-detail">
      ...
      <div className="tracker-stats-row">
        <div className="tracker-stat-card">
          ...
          <span className="tracker-stat-value">{formatResponseTime(monitor.last_response_time_ms)}</span>
        </div>
        <div className="tracker-stat-card">
      ...
      {openIncidents.length > 0 && (
        <div className="tracker-incidents-section">
          <h2 className="tracker-section-heading tracker-down-title">Active Incidents</h2>
          {openIncidents.map(incident => (
            <div key={incident.id} className="tracker-incident tracker-incident-open">
              <div className="tracker-incident-header">
                <span className="tracker-status-dot" style={{ background: 'var(--color-danger, #ef4444)' }} />
                <span className="tracker-incident-cause">{incident.cause ?? 'Site unreachable'}</span>
              </div>
              <div className="tracker-incident-meta">
                Started: {new Date(incident.started_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}
          ...
          {resolvedIncidents.map(incident => {
            ...
            return (
              <div key={incident.id} className="tracker-incident">
                <div className="tracker-incident-header">
                  <span className="tracker-status-dot" style={{ background: 'var(--text-muted, #94a3b8)' }} />
                  <span className="tracker-incident-cause">{incident.cause ?? 'Site was unreachable'}</span>
                </div>
                <div className="tracker-incident-meta">
                  {new Date(incident.started_at).toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' })}
                  {duration !== null ? ` — resolved after ${duration < 1 ? '<1' : duration} min` : ''}
                </div>
              </div>
            )
          })}
      ...
      <div className="tracker-subscribe-section">
        ...
        <p className="tracker-subscribe-desc">
          Enter your email to receive alerts when {monitor.display_name} goes down or recovers.
        </p>
      ...
      <div className="tracker-cta-section">
        ...
        <p>
          Get instant uptime alerts, performance metrics, and status pages for your own sites.
          Free plan includes 5 monitors with 5-minute checks.
        </p>
      ...
      <div className="tracker-disclaimer">
        <p>
          This page shows automated checks from Uptrue&apos;s monitoring infrastructure.
          Status may not reflect your local experience. Results are provided for informational
          purposes only. Uptrue is not affiliated with {monitor.display_name}.
        </p>
      </div>
    </div>
  )
