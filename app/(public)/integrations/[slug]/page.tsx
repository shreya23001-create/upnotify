...
const PAGES: IntegrationPage[] = [
  {
    ...
    name: 'Slack',
    emoji: '💬',
    heroTitle: 'Upnotify + Slack — Real-time Alerts in Your Team Channel',
    ...
    seoTitle: 'Upnotify Slack Integration — Website Monitoring Alerts in Slack',
    seoDescription: 'Send Upnotify uptime, SSL, DNS and performance alerts directly to Slack. Block-formatted messages with severity, target and one-click View Monitor button. Set up with an incoming webhook in 60 seconds.',
    howItWorks: [
      'When a monitor goes down, Upnotify runs a second confirmation check 5 seconds later. If both checks fail, an incident opens and an alert is dispatched to every channel you have configured — including Slack.',
      'The Slack message uses Slack\'s native Block Kit formatting: a clear headline, severity / target / status / time fields, and a one-click "View Monitor" button that takes the on-call engineer straight to the monitor in your Upnotify dashboard.',
    ],
    setupSteps: [
      ...
      'In Upnotify, open Dashboard → Alerts → New Channel, pick "Slack" as the type, and paste the webhook URL.',
    ...
    whatYouGet: [
      ...
      'One-click "View Monitor" button linking to the monitor in your Upnotify dashboard',
  ...
  },
  {
    ...
    emoji: '👥',
    heroTitle: 'Upnotify + Microsoft Teams — Alerts in Your Team Channel',
    ...
    seoTitle: 'Upnotify Microsoft Teams Integration — Website Monitoring Alerts in Teams',
    seoDescription: 'Send Upnotify website monitoring alerts to Microsoft Teams. MessageCard formatting with red/green theme colour, severity, target, status, and a one-click View Monitor action. Set up with an Incoming Webhook connector in 60 seconds.',
    howItWorks: [
      'When a monitor confirms a failure, Upnotify dispatches an alert to your Teams channel using the standard Office 365 Connector MessageCard format. Resolved incidents fire as a green-themed message; new incidents fire as red.',
      'Each alert includes the incident title, severity, monitored target, and current status — plus an "OpenUri" action that takes the on-call engineer straight to the monitor detail view in your Upnotify dashboard.',
    ],
    setupSteps: [
      'In Microsoft Teams, open the channel you want to receive alerts in, click the three-dot menu → Connectors.',
      'Find "Incoming Webhook", click Configure, give it a name (e.g. "Upnotify Alerts"), optionally upload an icon, and click Create. Teams gives you a webhook URL.',
      'In Upnotify, open Dashboard → Alerts → New Channel, pick "Microsoft Teams" as the type, and paste the webhook URL.',
    ...
    faq: [
      ...
      { q: 'What happens if Teams is unreachable when an alert fires?', a: 'The alert is recorded as failed in the alerts table and the dispatch error is logged. Upnotify does not retry the dispatch. Best practice: pair Teams with at least one other channel (email, signed webhook) for redundancy on critical infrastructure.' },
    ],
  ...
  },
  {
    ...
    name: 'Telegram',
    emoji: '✈️',
    heroTitle: 'Upnotify + Telegram — Push Alerts to Your Phone',
    ...
    seoTitle: 'Upnotify Telegram Integration — Website Monitoring Alerts on Telegram',
    seoDescription: 'Send Upnotify uptime, SSL and DNS alerts to a Telegram chat or channel. Mobile-first push notifications, severity-coded messages, one-click View Monitor link. Set up with a chat ID — no bot installation required by the user.',
    howItWorks: [
      'Upnotify runs a managed Telegram bot. When you set up a Telegram channel inside Upnotify, you give it the Telegram chat ID where you want alerts delivered. The Upnotify bot then posts every alert into that chat.',
      'You retain full control: invite or remove the Upnotify bot from any chat at any time. Alerts are formatted with the monitor name, target, severity, and a deep link to the monitor inside your Upnotify dashboard.',
    ],
    setupSteps: [
      'Add the Upnotify Telegram bot to the chat or channel where you want alerts delivered (the bot username is shown in your Upnotify dashboard once you start setup).',
      ...
      'In Upnotify, open Dashboard → Alerts → New Channel, pick "Telegram" as the type, and paste the chat ID.',
    ...
    whatYouGet: [
      ...
      'Deep link to the monitor detail view in your Upnotify dashboard',
  ...
  },
  {
    ...
    name: 'Webhook',
    emoji: '🪝',
    heroTitle: 'Upnotify + Signed Webhooks — Wire Alerts Into Anything',
    heroSubtitle: 'Receive every Upnotify alert as a JSON POST to your endpoint, signed with HMAC-SHA256. Pipe into PagerDuty, Opsgenie, your own incident management, or anywhere else.',
...

{
  ...
  "timestamp": "2026-05-03T12:34:58.123Z"
    ...
    faq: [
      ...
      { q: 'What happens if my endpoint is down when an alert fires?', a: 'The alert is recorded as failed and the dispatch error is logged in your alerts table. Upnotify does not currently retry — pair the webhook with at least one other channel (email, Slack, Telegram) on critical infrastructure for redundancy.' },
    ],
      ...
      <main style={{ maxWidth: 880, margin: '0 auto', padding: '52px 24px 80px' }}>
        ...
        <section style={{ marginBottom: 56 }}>
          ...
          <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 20 }}>
            Every Upnotify monitor type can send to {page.name}. Some of the most common:
          </p>
