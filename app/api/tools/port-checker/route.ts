...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!ALLOWED_PORTS.has(port)) {
      return NextResponse.json(
        {
          error: 'Port not allowed. The Upnotify port checker only supports common web and mail ports — see https://upnotify-monitoring.vercel.app/help/tools for the full list.',
        },
        { status: 400 }
      )
    }
