...
interface CalcCardProps {
  onClose?: () => void  // present = popup mode, shows ✕
}

export function DowntimeCalculatorCard({ onClose }: CalcCardProps): React.ReactElement {
  const [revenue, setRevenue] = useState(50000)
  const [hours, setHours] = useState(2)
  const cardRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (onClose) return // skip buzz in popup mode
  ...
  const calc = useCallback(() => {
    ...
    const churn = revLoss * 0.4
    const rep = revLoss * 0.6
  ...
  return (
    <div className="calculator-card" ref={cardRef} style={{ position: 'relative' }}>

      {/* ✕ close button — only in popup mode */}
      {onClose && (
        <button className="calc-popup-close" onClick={onClose} aria-label="Close">
