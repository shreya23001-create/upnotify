...
- **Razorpay webhook signature always verified when secret set (engineering-app#62).** Pre-fix: dev/staging accepted unsigned webhook payloads — anyone could POST a fake `subscription.activated` to dev.uptrue.io and forge a plan change against any org id. Now the env name is no longer the signal; the presence of `RAZORPAY_WEBHOOK_SECRET` is. If the secret is set, every request is verified regardless of env. If it's not set, the route returns 503 (fail closed). Also hardened the verifier itself — added length-check before `crypto.timingSafeEqual` so a missing signature no longer 500s the route.
