...
- **security.txt added (engineering-app#93).** RFC 9116 responsible-disclosure file at `/.well-known/security.txt`. Points researchers at `shreya23001@gmail.com` (Boss needs to confirm/set up the alias) and the existing `/security` policy page.
- **Razorpay webhook signature always verified when secret set (engineering-app#62).** Pre-fix: dev/staging accepted unsigned webhook payloads — anyone could POST a fake `subscription.activated` to upnotify-monitoring.vercel.app and forge a plan change against any org id. Now the env name is no longer the signal; the presence of `RAZORPAY_WEBHOOK_SECRET` is. If the secret is set, every request is verified regardless of env. If it's not set, the route returns 503 (fail closed). Also hardened the verifier itself — added length-check before `crypto.timingSafeEqual` so a missing signature no longer 500s the route.
- **Port checker locked to a 13-port allowlist (engineering-app#63).** Removed open 1–65535 range. SSH (22), MySQL (3306), PostgreSQL (5432), Redis (6379), MongoDB (27017) no longer reachable through Uptrue. Allowed: 21, 25, 53, 80, 110, 143, 443, 465, 587, 993, 995, 8080, 8443. Stops Uptrue being used as an anonymous port scanner against third-party hosts.
- **Per-user burst rate-limit on citation-check (engineering-app#86).** Added `checkRateLimitByKey()` so the rate limiter can scope by arbitrary key (e.g. user id) instead of just IP. New preset `AI_CITATION_USER_RATE_LIMIT` (3 requests per minute per user). Stacks on the existing per-IP cost guard from #71 — narrows the race-condition window from "10 tabs at once" to "1 request per 20s per user", so a single bored user can no longer race the plan-monthly counter and burn shared engine API quota. Stayed in-memory rather than adopting Upstash Redis (no paid services until 500 paying customers).
- **Stored XSS in blog approval page (engineering-app#61, SEC-03 HIGH).** Both `successPage()` and `errorPage()` in `/api/admin/blog-approve` now route their heading + message arguments through `escapeHtml()` inside the template function, so the auto-blog title — which is DB-controlled and reaches the admin's browser via the approval email link — can't ship arbitrary `<script>` / `onerror=` payloads.
- **AI prompt input sanitisation (engineering-app#81, #83).** New `lib/utils/sanitize-ai-input.ts` with `cleanDomainForAi()` and `cleanKeywordForAi()` helpers — strip control chars (`\r \n \t`), LLM delimiter tokens (`< > " ' \``), cap length at RFC 1035 (253) for domains / 200 for keywords. Wired into all three AI routes (citation-check, profile-check, generate-llms). 13 unit tests cover newline injection, delimiter strip, length cap, and the exact payloads from Kriti's issue. Replaces five ad-hoc per-route sanitisation chains.
...
- **Dev/preview noindex hardening (engineering-app#90).** Per the hard SEO rule that only `upnotify-monitoring.vercel.app` is indexable, non-production environments now emit (1) `Disallow: /` robots.txt — shipped with #84, (2) `<meta name="robots" content="noindex,nofollow">` in `app/layout.tsx` gated on `process.env.VERCEL_ENV`, (3) `X-Robots-Tag: noindex, nofollow` HTTP response header from `next.config.ts`. Three-layer defence covers meta-tag rendering, 30x redirects, JSON/image responses, and any path the meta tag never reaches.
...
- **Monitor stats route — reject malformed IDs (engineering-app#72).** `GET /api/v1/monitors/[id]/stats` now validates the `id` param against a UUID regex at the top of the handler — 400 for non-UUID shapes (previously they reached Supabase and failed there). Org-id mismatch still returns 404. Defence-in-depth — the same pattern should be applied to other `[id]` routes as a separate follow-up.
- **Support ticket upload — clearer error on missing bucket (engineering-app#49).** `Bucket not found` errors from Supabase Storage now return 503 with a friendly "uploads temporarily unavailable, please paste or email" message. The root cause is operational: the `support-attachments` bucket is not created by any migration and Boss needs to provision it in the Supabase dashboard before user uploads can succeed.

### Performance
- **ISR site-wide; force-dynamic cascade removed (engineering-app#69, closes #65, #66, #67, #68).** The Vercel load test (#69) capped the site at ~150 concurrent users — every public page was being SSR'd on every visit because `app/(public)/layout.tsx` had `export const dynamic = 'force-dynamic'` cascading onto every page, and `PublicNav` called `noStore()` which opted every page out of static rendering even where the layout didn't. Removed both. Replaced `noStore()` with `unstable_cache(..., { revalidate: 60, tags: ['nav'] })` so the CMS-driven nav still updates within 60s (or instantly via `revalidateTag('nav')`) but lets pages cache. Added explicit `revalidate` exports to the four dynamic pages: homepage 600s, blog index 300s, leaderboard 300s, tracker 60s. ~95 other static pages (monitoring/*, tools/*, integrations/*, legal pages, blog static .tsx) become naturally static when the parent stops forcing dynamic. Out of scope (separate ops): switch Supabase `DATABASE_URL` to the transaction pooler (port 6543) for multiplied connection capacity.

### DB Performance
- **Check-runner batch maintenance lookup (engineering-app#58).** The cron used to fire one DB query per monitor to check maintenance windows (`isMonitorInMaintenance` × N inside `Promise.allSettled`). At 100 monitors per tick that was 100 simultaneous queries against `maintenance_windows`. Wired up the existing-but-unused `getMaintenanceSetForMonitors()` helper that does it in one query and returns a Set.
- **Monitors list bar chart truncation (engineering-app#57).** `getUptimeBarData()` had no `.limit()`. PostgREST silently caps unbounded selects at 1,000 rows; a 1-minute monitor over 24 hours has 1,440. Bumped to `.limit(1440)` so the bar chart shows the full window.
- **Dashboard stats payload (engineering-app#59).** `getRecentCheckResultsByMonitorIds()` and `getRecentCheckResultsByOrg()` used `.select('*')` returning the 2-5 KB metadata JSONB column per row at 5,000 rows = ~15 MB transferred. Projected select to the 4 fields the dashboard actually renders (`status, response_time_ms, checked_at, monitor_id`) and introduced a `DashboardCheckResult` `Pick<>` type. ~30× payload reduction.
...
- Takedown/corrections email: shreya23001@gmail.com
