...
// =============================================================================
// Queue-based generation (migration 00065)
// =============================================================================
...
export async function createQueuedAutoblogRun(input: {
  channel_key?: string | null
  topic_id?: string | null
  source_key?: string | null
  post_to_social?: boolean
  ...
  if (error) {
    logger.error('Failed to create queued autoblog run', { error: error.message })
    return null
  }
  return data?.id ?? null
}

export async function getNextQueuedRun(): Promise<QueuedRun | null> {
  const supabase = db()

  // Reset runs stuck in generating for more than 5 minutes (crash recovery)
  await supabase
    .from('autoblog_runs')
    .update({ status: 'queued' })
    .eq('status', 'generating')
    .lt('ran_at', new Date(Date.now() - 5 * 60 * 1000).toISOString())

  const { data, error } = await supabase
    .from('autoblog_runs')
    .select('id, channel_key, topic_id, source_key, post_to_social, confidence_score, sources_count, payload, ran_at')
    .eq('status', 'queued')
...
export async function updateAutoblogRunResult(id: string, result: {
  ...
  await supabase
    ...
    .update({
      ...
      blog_post_id: result.blog_post_id ?? null,
