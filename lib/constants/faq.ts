...
export const FAQ_ITEMS: FaqItem[] = [
  {
    ...
    answer: 'When a check detects a potential issue, Upnotify waits 30 seconds and runs a second check from a different region. An incident is only created if both checks confirm the problem. This eliminates false alarms caused by temporary network blips or routing issues.',
  },
  {
    question: 'What monitor types does Upnotify support for website uptime monitoring?',
    answer: 'Upnotify supports 24 monitor types covering uptime, security, and infrastructure: HTTP/HTTPS uptime, SSL certificate expiry, DNS record changes, keyword presence, domain expiry, port availability, ping/ICMP, API endpoint validation, heartbeat (cron job monitoring), page change detection, security headers, response time, robots.txt, IP change, MX health, WHOIS/registrar, sitemap, redirect chain, SPF/DMARC, blacklist, page size, cookie consent, nameserver monitoring, and Upnotify WordPress Monitor for site-internal scanning.',
  },
  {
  ...
  },
  {
    ...
    answer: 'Upnotify sends alerts via email, Slack, Microsoft Teams, Telegram, and webhooks. Webhook payloads are signed with HMAC-SHA256 for security. You can configure multiple channels per monitor and filter by severity.',
  },
  {
