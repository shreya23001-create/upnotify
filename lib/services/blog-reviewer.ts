...
export async function reviewDraft(input: ReviewInput): Promise<ReviewOutput> {
  ...
  try {
    const message = await client.messages.create({
      // Haiku is plenty for this short structured-summary task — ~5x cheaper
      // than Sonnet, no quality risk on 200-word digest output. Generator
      // (long-form post) stays on Sonnet — quality matters more there.
