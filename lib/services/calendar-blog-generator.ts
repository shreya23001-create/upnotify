/**
 * Calendar-driven blog generator — v2 of the autoblog pipeline.
 *
 * Pulls a row from content_calendar, generates a draft via Claude with
 * post-type specific prompting, validates Definition of Done, runs the
 * reviewer pass, saves to blog_posts with delivery_method='digest'.
 *
 * Coexists with v1 (autoblog-generator.ts) — v1 handles outage / LLM news /
 * custom topic with immediate per-post emails. v2 handles calendar-driven
 * content destined for the daily Boss Digest.
 */
