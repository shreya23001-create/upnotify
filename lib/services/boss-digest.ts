/**
 * Boss Digest — daily 07:00 email batching all pending calendar drafts
 * into a single email with mini-cards.
 *
 * One call to buildAndSendBossDigest():
 *   1. Fetch all blog_posts where delivery_method='digest', digest_status='pending'
 *   2. Render single email with N mini-cards (highlights, worries, approve/reject)
 *   3. Send via Resend
 *   4. Mark all sent posts as digest_status='in_digest', digested_at=now()
 *   5. Insert audit row in boss_digest_runs
 *
 * Approve/reject reuses the existing /api/admin/blog-approve handler — no
